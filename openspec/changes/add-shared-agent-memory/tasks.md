...
- [x] 1.6 Add an idempotent migration from the reserved title `Household overview` to `Overview`; advance its revision while preserving its stable ID, body, tags, creation time, and source metadata, and verify fresh databases seed the new title.
...
- [x] 2.4 Replace memory-specific model guidance and context labels with natural `shared memory`, `notes`, and `Overview` wording while leaving the broader household-assistant base prompt unchanged; verify built-in and custom prompt composition.
...
- [x] 4.4 Render concise `Memory — page <n>` and `overview — Overview` labels, including exact overview reads and unavailable-memory errors, without changing command syntax, access, pagination, or literal body delivery.
...
- [x] 6.3 Update user/operator documentation and tests for the natural memory terminology, confirm architecture-only uses of `household` remain intentional, then rerun the complete project checks and strict OpenSpec validation.
